...
# Define useful for MMM functions -----------------------------------------
    ...
    MMM_optimize_budget_split <- function(max_investments, revenue_coef, response_curves, rest_dict){
      
      # ineqLB_func = function(rest_dict){
      #   min_rest = c()
      #   for (name in names(rest_dict)){
      #     min_rest = c(min_rest, rest_dict[[name]][[1]])
      #   }
      #   return(min_rest)
      # }
      # 
      # ineqLB_vals <- ineqLB_func(rest_dict)
      # 
      # ineqUB_func = function(rest_dict){
      #   max_rest = c()
      #   for (name in names(rest_dict)){
      #     max_rest = c(max_rest, rest_dict[[name]][[2]])
      #   }
      #   return(max_rest)
      # }
      # 
      # ineqUB_vals <- ineqUB_func(rest_dict)
      # 
      # Calculate optimal split for all budget sizes
      
      min_rest_0 <- 0
      for (name in names(rest_dict)){
        min_rest_0 <- min_rest_0 + rest_dict[[name]][[1]]
      }
      
      max_rest_0 <- 0
      for (name in names(rest_dict)){
        max_rest_0 <- max_rest_0 + rest_dict[[name]][[2]]
      }
      if (max_investments < max_rest_0) {
        max_rest_0 <- max_investments
      }
      
      print('min_rest_0')
      print(min_rest_0)
      splits <- pblapply(seq(min_rest_0 + 10^4, max_rest_0, 10^4), function(budg, coef, response_curves1, rest_dict){
        
        ineqLB_func = function(rest_dict, budg){
          min_rest = c()
          for (name in names(rest_dict)){
            min_rest = c(min_rest, (rest_dict[[name]][[1]])/(budg+100))
          }
          return(min_rest)
        }
        
        ineqLB_vals <- ineqLB_func(rest_dict, budg)
        
        
        print('ineqLB_vals')
        print(ineqLB_vals)
        
        ineqUB_func = function(rest_dict, budg){
          max_rest = c()
          for (name in names(rest_dict)){
            max_rest = c(max_rest, (rest_dict[[name]][[2]])/(budg+100))
          }
          return(max_rest)
        }
        
        ineqUB_vals <- ineqUB_func(rest_dict, budg)
        ...
        # Set optimisation function
        optimum <- solnp(
          pars = rep(0.001, length(coef$media)),
          fun = optimize_budget,
          ...
          LB = ineqLB_vals,
          UB = ineqUB_vals,
          # ineqfun = function(x, budget, coefficients, response_curves){
          #   ivst = c()
          #   z = c()
          #   for (i in seq(1,length(rest_dict))){
          #     z[i] = x[i]*budget
          #     ivst = c(ivst, z[i])
          #     }
          #   return(ivst)
          ...
          #control = list(outer.iter = 5, inner.iter=5, rho=10, tol=1e-2, delta=1.0e-1)
          control = list(delta=1.0e-8)
        )
      ...
      }, coef = revenue_coef, response_curves1=response_curves, rest_dict=rest_dict) %>% rbindlist(fill=T)
    ...
    # Function optimizing budgets for channels for given media budget
    MMM_optimize_budget_split_2 <- function(max_investments, revenue_coef, response_curves, rest_dict){
      ...
      # 
      # Calculate optimal split for all budget sizes
      splits <- pblapply(seq(10^5, max_investments, 10^5), function(budg, coef, response_curves1, rest_dict){
        ...
        # Set optimisation function
        optimum <- solnp(
          pars = rep(0.01, length(coef$media)),
          fun = optimize_budget,
          eqfun = function(x, budget, coefficients, response_curves){sum(x)}, # sum of weights
          eqB = 1, # sum of weights equals 1
          LB = rep(0, length(coef$media)),
