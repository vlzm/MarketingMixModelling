...
HOLIDAYS = [
    ...
    'Valentins_day_neg',
    'Valentins_day_pos',
    'Birthday_of_Martin_Luther_King',
    'Independence_Day',
    'Columbus_Day',
    'Veterans_Day',
    'Thanksgiving_Day',
...
def get_responses(revenue_table):
    ...
    return (
        ...
        for temperature in ['_hot', '_warm', '_cold']
    )
...
#         for i, width in zip(range(startcol, startcol + len(df.columns)), get_col_widths(df)):
#             writer.sheets['media_impacts_abs'].set_column(i, i, width)
        ...
        for temperature in ['Cold', 'Warm', 'Hot', 'Overall']:
            df[stat_type][temperature] = (df[stat_type][temperature] / df[stat_type]['Overall'].sum() * 100).round(1)
...
def add_decomposition_table(decomposition):
    ...
    covid = decomposition.filter(like='covid')
...
#     impressions = impressions.reset_index()
    ranges = {
         '2019': pd.date_range('02-01-2019', '12-31-2019'),
...
with pd.ExcelWriter('2ew_results.xlsx') as writer:
